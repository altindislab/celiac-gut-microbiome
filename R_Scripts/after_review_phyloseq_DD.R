...
#```{r parse}
# meta data
md_old <- read.csv("../CD_new/data/CeD new label (2).csv")
md_age <- read.csv("../CD_new/data/Celiakibarn 190906.csv")
md_old <- merge(md_old, md_age, all = TRUE)
rm(md_age)

##DD removed age 1 samples
...
md2 <- subset(md,Age==2.5)
md2_filtered <- subset(md2, ABIS.no!="19582")

##DD removed age 5 samples (19582, 8743, 17974, 22068, 3933)
md5 <- subset(md,Age==5.0)
md5_filtered <- subset(md5, ABIS.no!="19582") ##No sample at this age already
md5_filtered <- subset(md5_filtered, ABIS.no!="8743")
md5_filtered <- subset(md5_filtered, ABIS.no!="17974")
md5_filtered <- subset(md5_filtered, ABIS.no!="22068")
md5_filtered <- subset(md5_filtered, ABIS.no!="3933")

##DD Combine filtered md
md <- rbind(md2_filtered,md5_filtered)
...
## otu
seqtab.nochim <- readRDS("../CD_new/dada2/seqtab_nochim.rds")
dim(seqtab.nochim)
...
# taxa
taxa <- readRDS("../CD_new/dada2/annot_add_species.rds")
...
for (s in unique(sample_data(nps1)$Sort)) {
  for (a in unique(sample_data(nps1)$Age)) {
    ...
    aov.tab <- adonis(otu_table(nps1.sub) ~ Disease.status, data = data.frame(sample_data(nps1.sub)), perm = 1e4-1, method = "bray")$aov.tab
